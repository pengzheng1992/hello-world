...
int more_free_place(IN OUT Flow & f)
{
	...
	for (auto it = f.switchesPath.begin(); it != f.switchesPath.end(); ++it) {
		if (sid == it->first) {
			...
			assert((ps->counters.size() + 1) <= ps->flowTableCountingRuleEntriesTotal);
			assert((ps->counters.size() + 1) * ps->counterWidth <= ps->counterMemoryTotal);
...
int random_place(IN OUT Flow & f)
{
	...
	shuffle(ss.begin(), ss.end(), std::default_random_engine(seed));
	//sort(ss.begin(), ss.end(), moreEmpty());
	if (ss.size() == 0) {
		//what if all switches has rules
		assert(false);
