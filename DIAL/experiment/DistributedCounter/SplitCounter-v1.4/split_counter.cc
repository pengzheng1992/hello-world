...
//static bool ns8s[256];

void network_segment(IN OUT struct Packet *p_packet) {
	//cout << (p_packet->tuple5.saddr >> 24) << " " << (p_packet->tuple5.daddr >> 24) << endl;
...
// 		exit(0);
	...
	/*if (p_packet->s_net_segment > 35) {
		p_packet->s_net_segment = 35;
	}
	if (p_packet->d_net_segment > 35) {
		p_packet->d_net_segment = 35;
	}*/
	...
	//	// 		exit(0);
	//}
	//int sns8 = p_packet->tuple5.saddr >> 24;
	//int dns8 = p_packet->tuple5.daddr >> 24;
	//if (p_packet->flow)
	//{
	//	if (!ns8s[sns8])
	//	{
